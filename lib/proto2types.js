...
function proto2types(options) {
    let {
        ...
        // Message types eventually become become tables whose primary key
    ...
    const typesByName = protoInfo.protoFile.reduce((typesByName, file) => {
        ...
        (file.messageType || [])
            ...
            .map(message => message2type(packageName, message, idFields))
            .forEach(type => typesByName[type.name] = type);

        (file.enumType || [])
            .map(anEnum => enum2type(packageName, anEnum))
            .forEach(type => typesByName[type.name] = type);
    ...
    // Make sure that each resulting object satisfies schemas/type.tisch.js
    const isValid = schemas.type;
    results.forEach(type => {
        if (!isValid(type)) {
            throw Error("There's a bug in this program. "  +
                        'Output object does not satsify the "type" schema:\n' +
                        isValid.errors.join('\n'));
        }
    });
...
// "id" field.
function message2type(packageName, descriptor, idFields) {
...
// (and its plugins).
function enum2type(packageName, descriptor) {
    return withDocs(descriptor.location, {
        kind: 'enum',
...
function builtinMessage(typeName) {
    return ['.google.protobuf.Timestamp'].includes(typeName);
}
...
function location2docs(location) {
    return [
        'leadingDetachedComments', 'leadingComments', 'trailingComments'
...
function invokeProtocJson(protoIncludePaths, protoFiles) {
    ...
    // doesn't complain. I haven't figured out what makes the difference. To
    // make it work, add the parent directory for each .proto file.
