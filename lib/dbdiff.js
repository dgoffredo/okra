...
function diffRows(tableBefore, tableAfter) {
    const primaryKeyColumnName =
        tableBefore.primaryKey && tableBefore.primaryKey.length === 1
            ? tableBefore.primaryKey[0]
            : undefined;
    const primaryKeyColumnIndex = tableBefore.columns.findIndex(
        column => column.name === primaryKeyColumnName);
    ...
    // Some tables don't have a one-column primary key (array field tables).
    // In that case, the table definitions must also have no rows.
    // That is, array field tables never have rows generated by the schema. Only
    // enum tables have rows generated by the schema.
