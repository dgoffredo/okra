...
function dbdiff(tablesBefore, tablesAfter) {
    ...
    Object.values(tablesBefore).forEach(schemas.table.enforce);
    Object.values(tablesAfter).forEach(schemas.table.enforce);
...
function diffDefinition(tableBefore, tableAfter) {
    ...
    tableAfter.columns.slice(0, mismatchIndex).forEach((column, i) => {
        ...
        let isAltered = false; // `true` if we find anything different

        ['type', 'description'].forEach(property => {
            // types and descriptions are strings, so we can do normal comparison
            if (column[property] !== beforeColumn[property]) {
                isAltered = true;
                alteration[property] = column[property];
            }
        });
